...
describe('Teste das rotas do servidor', () => {
  let server: any;
  const userData = {
    nome: 'Teste',
  ...
  it('Deve responder com status 400 para a rota de registro se o email for inválido', async () => {
  ...
  it('Deve responder com status 409 para a rota de registro se o email já estiver cadastrado', async () => {
  ...
  it('Deve responder com status 401 para a rota de login se as credenciais forem inválidas', async () => {
    const res = await request(app)
      .post('/login')
  ...
  it('Deve responder com status 401 para a rota de login se o email não estiver cadastrado', async () => {
    const res = await request(app)
      .post('/login')
  ...
  it('Deve responder com status 400 para a rota de avaliar professor se algum campo estiver faltando', async () => {
    const res = await request(app)
      .post('/avaliar-professor')
      .send({
        professorNome: 'Professor Teste',
  ...
  it('Deve responder com status 400 para a rota de avaliar professor se a nota for inválida', async () => {
    const res = await request(app)
      .post('/avaliar-professor')
      .set('Authorization', `eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MSwiZW1haWwiOiJhQGEuY29tIiwiaWF0IjoxNzE3ODczMjIxLCJleHAiOjE3MTc4NzY4MjF9.7Y6hXCDjgvzPESRee0rYNpmu83VyQ6ahdzzvxPT_Js8`)
      .send({
  ...
  it('Deve responder com status 401 para a rota de avaliar professor se o token não for fornecido', async () => {
    const res = await request(app)
      .post('/avaliar-professor')
      .send({
        professorNome: 'Professor Teste',
  ...
  it('Deve responder com status 401 para a rota de avaliar professor se o token for inválido', async () => {
    const res = await request(app)
      .post('/avaliar-professor')
  ...
  it('Deve responder com status 401 para a rota de aluno se o token não for fornecido', async () => {
    const res = await request(app).get('/aluno');
    expect(res.statusCode).toEqual(401);
  });

  it('Deve responder com status 401 para a rota de aluno se o token for inválido', async () => {
    const res = await request(app)
      .get('/aluno')
      .set('authorization', 'token_invalido');
    expect(res.statusCode).toEqual(401);
